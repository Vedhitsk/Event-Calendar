...
//add eventlinstner on prev and next
...
addEventFrom.addEventListener("input", (e) => {
  ...
  //if two numbers enetered auto add ":"
...
function updateEvents(date) {
  ...
  //save events when new one added or update
  saveEvents();
}
...
addEventsSubmit.addEventListener("click", () => {
  ...
  if (eventsArr.length > 0) {
    //check if current day has already any evvent then add to that
...
//lets create a function to remove on click
eventsContainer.addEventListener("click", (e) => {
  if (e.target.classList.contains("event")) {
    const eventTitle = e.target.children[0].children[1].innerHTML;
    //get the title of event than search in array by title and deleted
    eventsArr.forEach((event) => {
      if (
        event.day === activeDay &&
        event.month === month + 1 &&
